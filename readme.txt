...
I use a hash map to cache the longest name chain result starting from each node. If that node is visited again, then we simply return the cached result instead of doing DFS again. So that each node and each edge is visited once and the running time is O(N + M).

When there are cycles existing in the graph, the running time can reach exponential time in worst case(imagine a complete graph). This is because my algorithm cannot cache the longest name chain starting from node if that node is in the cycle. If very few node among many nodes are in a cycle, then the running time is still close to O(N + M). In the worst case, when the graph is a complete graph, that is every pair of nodes is connected by a bidirectional edge, then this is a NP-hard problem, which can be reduced from Hamiltonian path problem. (Refer from https://en.wikipedia.org/wiki/Longest_path_problem)


3.Unit test:

I have 3 unit tests for the important main function and helper class. 3 all passed. The coverage rate is 100% (the code for "main" is not tested), analyzed by EclEmma plug-in in Eclipse.

In order to run the unit test, JUnit and Mockito are required. I used Unit 4 and mockito-all-1.9.5.jar.

4.Functional test:
Functional testing passed.
Two small input file are used for the functional testing. 
"test/celebrity/name/game/test/sample.in" is the test input which can be modeled into a graph with no cycle.
"test/celebrity/name/game/test/sample2.in" is the test input which can be modeled into a graph with cycle.
